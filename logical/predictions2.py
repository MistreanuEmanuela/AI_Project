...
def data_view_per_class(data, target):
    ...
    plt.ylabel('Count')
    plt.savefig("./diagrams2/nr_inst_classes.png")
    plt.close()
...
def plot_rn_mae(history):
    ...
    plt.legend()
    plt.savefig("./diagrams2/mean_error.png")
    plt.close()
...
def plot_rn_accuracy_continuous(history):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig("./diagrams2/accuracy_test.png")
    plt.close()
...
def plot_rn_ROC(model, x_test, num_classes, y_test):
    ...
    plt.savefig("./diagrams2/roc_neural.png")
    plt.close()
...
def plot_test_accuracies_comparison(accuracy_id3, accuracy_RN_1, accuracy_RN_2, accuracy_bayes,
    ...
    plt.savefig("./diagrams2/accuracy_comparison.png")
    plt.close()
...
def predictions2():
    X, Y = load_data("./data/Preprocessing_HHCAHPS.csv", 'HHCAHPS Survey Summary Star Rating')
    print(X, Y)
    label_encoder = LabelEncoder()
    ...
    plot_rn_ROC(RN_model2, X_test_RN2, 4, y_test_RN2)
